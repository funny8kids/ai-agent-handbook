...
*《图：本章反复回到的一张图——圈只有四步，差别全在「规划」这一步让模型自主到什么程度》*
## 先看结论
...
- **循环真正跑起来时**：前几轮看着都正常，出问题多在第八到第十五轮——观察被截断之后，模型开始重复上一步动作。经验值上，一个正常任务五到十二轮就该收敛；跑到二十轮还没完，多半不是在聪明地绕路，是在烧钱。真该跑几十轮的长任务也有，但那种情况你得说清每一轮在往前推进什么。
- **止损开关先于聪明**：上线后第一周补的通常不是更聪明的提示词，而是三道门——轮次上限、单次工具超时、整任务 token 预算。我们的项目里，加齐这三道之后失控类告警通常能少掉大半。
- **演示台与生产台的分界**：{% hint style="warning" %}现场最容易翻车的不是模型不会做事，而是它把「用户没回话」读成「用户同意了」。判据还是本文那两条：这一步由谁决定、工具由谁挑。答不上来的系统，名字里带不带 Agent 都不要紧。{% endhint %}
