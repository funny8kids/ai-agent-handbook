...
![上下文窗口预算分配：稳定前缀在前，压缩只动历史与观察](../.gitbook/assets/03-context-budget.svg)

*《图：200k 窗口切成五块，历史对话独占 45%，所以压缩先拿它开刀；稳定前缀排最前是为了不吃缓存》*
## 问题动机
...
flowchart TB
  W["上下文窗口<br/>（如 200K）"] --> A["System Prompt<br/>固定 ~2-10K"]
  W --> B["项目记忆<br/>CLAUDE.md/工具说明<br/>（~1-5K）"]
  W --> C["对话历史+工具结果<br/>（持续膨胀）"]
  W --> D["输出预留<br/>~4-16K"]
  C -.快满时.-> E["压缩/摘要/截断"]
```

*《图：200K 窗口拆成系统提示、项目记忆、历史与工具结果、输出预留四块——只有第三块随轮数单调膨胀，虚线也唯一地由它触发，快满就压缩/摘要/截断》*
