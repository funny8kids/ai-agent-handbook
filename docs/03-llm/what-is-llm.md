...
- **无状态落在账单上**：每一轮都得把历史整段重发。我们常见的项目里，第十轮的输入 token 往往是第一轮的四到六倍，而真正新增的信息只有一条工具结果——压缩与摘要的收益就藏在这道差里，这也是上下文工程最先见效的地方。
- **概率性有多概率**：同一段 prompt、同一份输入连跑二十次，经验值上十来次完全一致，不一致的几乎全集中在格式和「这一步到底调不调工具」。所以校验写在代码里不是保守，是唯一守得住的地方；把温度调低只是换了一种错法。
- **「会编」在 Agent 里更贵**：{% hint style="warning" %}问答场景编一个事实，用户看出来就走开了；Agent 场景编的是一个订单号、一段文件路径，后面三五轮全建立在它上面，回滚得按整轮算。经验值上，让取值一律来自工具返回、再由代码校验，能消掉大部分这类返工。{% endhint %}
