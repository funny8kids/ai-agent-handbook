...
- 抽取器点数与房内口径**逐位对齐**：54 个组件页、`steps=279 / tabs=195`。判据控制：3 组件真样本要解析对（含一个正文只有行内代码的 step）、围栏里的示例必须数为 0、被标点改写过的标题要命中、编造字符串必须不命中、`<script>` 里的文本不得算可见。
...
- SHAPE 判据最初用「markdown H1 原样子串」，把 6 个 lab 页误判成页面没上线；实际是标点被改写（`：` → 空格）。加 `norm()` 归一化后假警报消失，**真缺陷（标题发散）才浮出来**。

### 五、本轮验证与未做项

- **五把尺子全绿**：结构扫描 198 页 0 问题（frontmatter 四键齐、围栏配平、每页恰好一个 H1）；`check_nav_h1_sync.py` 196/0；`check_readme_stats.py` 全轴 ok；`check_widget_pairing.py` 198/0；`check_quote_fidelity.py` 197 页 4 处引用 0 不合格。
- **未做**：① 线上**整页真截图**目检仍受沙箱限制（Edge headless 无外网、内置浏览器超时），本轮只能证到「线上字节 + 线上 DOM 文本」；② 组件可见性本轮只抽 10 页，`--all` 全量 54 页留给推送后复跑；③ 第 54 轮列的第三件事——**引用逐字回核轴加宽**（按「同段落出现页面链接 + 引号」再扫一遍，并准备「日志里自造词」的假阳性控制）——本轮仍未做，是下一轮的第一件事。
