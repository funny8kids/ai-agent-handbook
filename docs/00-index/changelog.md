...
## 2026-09-23（第 56 次）新开「正文混进繁体字」轴并修掉 2 处；「引用逐字回核轴加宽」量成**负结果**，据此拒绝把它做成硬判据

上一轮收尾时列的第一件事（把引用轴加宽）本轮做了，结论是**这条轴不该加宽**；反倒顺手量到一条一直没人看的轴：**简繁混排**。两个缺陷都在提示卡里——读者扫一眼就看得到那种。

### 一、新轴：读者可见正文里不得出现繁体字形（第 7 把尺子）

- **量到多少**：全站 198 页、剥掉围栏与行内代码后有 **315,068 个汉字**（这是**开量时的读数**；本节写完后同一把尺子读到 316,644，多出来的全是本页新增的字），其中繁体字形 **2 处**：
  `05-tool-protocol/tool-permission-sandbox.md:19`「模型被 `騙` 着用合法权限做坏事」、`08-multi-agent/README.md:11`「协作能换 `來` 更强的能力面」。两处都在**一句话提示卡**里，线上原样渲染。
...
- **红→绿是量出来的，不是推的**：同一把尺子跑 `HEAD` 版本的这两个文件 → `findings=1 [('騙', 19)]` 与 `findings=1 [('來', 11)]`；跑修完的工作区 → **0**。最终全站读数 `pages=198 prose CJK chars=316644 traditional-form findings=0`。
...
- **七把尺子全绿**：`check_structure.py` 198/0；`check_nav_h1_sync.py` 196 页 0（含 SAMETITLE 轴）；`check_readme_stats.py` 全轴 ok；`check_widget_pairing.py` 198/0；`check_quote_fidelity.py` 197 页 4 处 0 不合格；**新** `check_char_sanity.py` 316,644 字 0 处繁体；`check_widget_visibility_live.py --all` 54 页 / 474 项 / 0 问题，且线上 URL 映射 **196/196 全覆盖**（上一轮 191/196）。
- **未做**：① 线上**整页真截图**目检仍受沙箱限制（Edge headless 无外网），本轮线上证据仍是「渲染后的 HTML 文本」；② 繁体字轴只覆盖「简繁两边确实不同形」的那一类，**形近错字**（如「需求/须求」）不在这条轴的射程里，单例字漏斗证明它需要人工；③ 若要把引用回核做成硬判据，需要先定句式约定（见第二节）。
