...
## 2026-09-23（第 65 次）开工·逐张重排手绘 SVG 的文字/画布比例：批 1 五张全部过线（223 个低于 12px 的标签清零），全站读数 32 张 / 956 个 → 27 张 / 734 个；线上抽查另抓到一条离线量不到的新轴——GitBook 的 sanitizer 会剥掉 `paint-order`，5 个「描边当光晕」的标签在读者眼里被自己的描边糊掉（已修，判据落库 `check_svg_sanitizer.py`）

本轮动正文 0 页、手绘 SVG 6 张（首页封面 + 4 张正文图重排，另 `19-lab-react-loop-animated` 只改光晕写法），判据 2 个文件（`check_svg_legibility.py` 的账目段；新增 `check_svg_sanitizer.py`）。第 64 轮把这条轴立起来之后，操作者定了三件事：**32 张分批逐张重排**、**更新日志页不拆（保持单页）**、**宽版布局去开**。本轮就是批 1；宽版那条复测过仍未生效（见五.3）。线上抽查又开出一条新轴，见六。
...
全站：**32 张 / 956 个 → 27 张 / 734 个**（Δ 正好是这五张的 223 个；标签总数 1042 → 1056，其中 +5 是六.2 那处光晕改法把 1 个 `<text>` 拆成 2 个——底下一层 `fill="none"` 只画描边，看不见但照旧计入，不为绿灯开后门）。`--column 1152` 的宽版 what-if 同步从 23 张 / 378 个降到 **18 张 / 204 个**。轴本身照旧退出码 1（还有 27 张没过），门槛没有往下挪一寸。
...
### 六、线上抽查抓到一条离线量不到的新轴：平台会把 `paint-order` 剥掉
