...
## 2026-09-26（第 100 次）面试章最薄三页增厚：显存与算子的细账、通用题池的判分口径、一条钻到底的追问链

按「面试内容还不够丰富」给本章最薄三页各加一组题，题题带回查题源：推理与服务化加「算子与显存的细账」5 题（14B 权重账、KV Cache 公式与量级、FlashAttention 三步落地、`contiguous()` 与 stride、KV 压缩四路线），手撕代码加「通用题池与追问」9 题（三数之和到 Agent 状态机），项目深挖在中间插进「追问链」6 题并把后面 13 题顺延重编号；章末自测补 7 条指向这些新节，新增引用 7 处探过后进外链缓存（两处 GitHub 源码跳转判 429，属于「判不了」而不是「死了」）。这一趟还查出四条首页统计停在第 95 轮之前的树上（含公式页数、章色块数、围栏分类，以及「知识/资源页都有可点开的出处」那整句），最后那句从来没有脚本复跑过，现在进了 `check_link_graph.py` 的 citation 腿。

## 2026-09-26（第 99 次）三条会说谎的尺子：短读的覆盖面、没人拉过的控制系统、首页那句没人盯的页数

29 MB 的更新日志页同一地址能返回三种大小，短读现在最多重试 6 次、并发那一趟被拒的页还要单独再取一次，两边都拿不到才落进 `fetch-failures` 卡退出码，并印出 `serial re-pass` 让覆盖面自己交代。新增 `run_battery.py --selftests` 第一次把每条轴的种植控制模式全量拉出来，当场抓到四条早就失效或在说谎的尺子：完整性判据的首页句还写「196 篇正文」（它自己读到 211，第 96 轮同步徽章统计句那一次漏了这一句）、章色判据写死的「不存在的章 20」在 20 章真的上线后什么也不检查、跨页重复轴的 `pages=191` 从来没有判据盯着（实测 206）——现在它由 `homepage_parity()` 钉住：页数走等号、单元数走地板，六根控制 P0–P5 逐一证明每个方向都会响；`check_updated_dates` 量出更新日志与首页各自印着一层旧日期（第 98 轮的收尾提交过了本地零点，正文改在 09-26 而页面说 09-25），随正文一起写回。

## 2026-09-25（第 98 次）术语表 18 行没到读者眼前该有的样子：14 行被当成一段带竖线的文字，4 行被砍掉整格术语

四处 `||` 粘连的行与两段「只有行带、没有表头」的行带，一共把 18 行术语降级成段落或直接吞掉——缺表头的两段各自并回上一张真表，粘连行拆开，两处重复术语并进规范行（两条解释都保留，一个字没删）；修法按同一页的渲染差分确认（表格行 111 → 127，六条抽到的术语从「只在原文出现一次」变成各自占一格），再拿两张真实渲染图逐条看过。这类缺陷离线「文字在不在」永远看不见，所以判据常驻进 `check_structure.py` 的新 `TBLORPHAN` 腿：展示公式里的竖线由两条轴共用的同一个分词器豁免（首趟就在 08 章误伤 2 行，改的是尺子不是书），绿色由种植反例加一条「把真页的表头删掉就必须响」的文件派生控制换来。线上复验：术语表那一页读者侧 127 行、381 格（357 个正文格加 24 个表头格）与本地判据逐格对平，线上另抽的 8 条术语全部到达；更新日志这一页三次完整副本都停在第 96 次（`.md` 端点已是 98），读者打开它看不到 97 与 98——这份 29 MB 的文档要不要拆开是产品级取舍，移交操作者。

## 2026-09-25（第 97 次）面试章按「还不够丰富」扩到 15 页：新增数据工程、评测与裁判、安全与红队、多模态、前沿与研究岗、岗位分档六页；顺带修掉该章 6 张超宽 Mermaid 图

题源与上一轮同一口径（公开题库、候选人面经、一手论文），页内「参考资料」只放本轮探到可达的地址；术语表加 21 行、标签索引加 3 条。图形账也一并结清：第 96 轮那四张横向流程图仍把标签压到 10px 以下，本轮连同新页的两张一起改成纵向堆叠，全站 Mermaid 几何超宽图 108→102，`20-interview` 这一章归零；剩下 102 张是第 62 轮起就写在首页的那笔版式取舍账，本轮没动。另修掉一处会让整轮判据静默失踪的尺子毛病：`run_battery.py` 在第 15 条轴上抛 `UnicodeEncodeError` 死掉（子进程按控制台代码页写日志，运行器按 UTF-8 解码拿到替换符，再往 GBK 控制台一印就炸），后面 20 条轴一条没跑而摘要毫无察觉——现在子进程统一被要求写 UTF-8，结果行打印前按目标编码往返替换一次，两个方向都种进 `--selftest`。

## 2026-09-25（第 96 次）新增第 20 章「面试真题」：9 页大厂 LLM/AI 岗真题，题题带回查来源；术语表挪到最后一章（21）

本轮按操作者要求补内容：`docs/20-interview/` 一页导读 + 八页知识题（基础 / 训练对齐 / RAG / Agent 工程 / 推理服务 / 系统设计 / 手撕 / 行为面），题目全部取自可点开核对的公开题库与面经，页内「参考资料」只放本轮探到 200 的地址。同时把术语表从第 15 章挪到全书最后一章 21（概念与速查后置），代价已量出：编号 15 空出未复用、该页线上 URL 因侧栏标题拼音化而改变（站内引用与更新日志里的两条历史链接均已重指）；另把 `run_battery.py` 的未知旗标改成红退出而不是「跑完整套电池」，并把 `check_readme_stats.py` 两条写死 196/257 的幻影控制改成按当前文本下种——数字一旦诚实挪位，写死的控制就静默失效。

## 2026-09-25（第 95 次）两条会说谎的退出码：`--watch` 的绿不等于读者拿到本轮（新增 `--reader-visible`），表格轴把「一页压根没量到」从内容缺陷里拆出来并逐页与作者侧对平

`check_live_sync.py` 现在把「平台同步了吗」与「读者打开的那一页翻到本轮了吗」分成两个入参：默认口径不变（第 83 轮实测的钉住只算 NOTE），`--reader-visible` 下读者页落后判 1、该腿读不到判 2（无结论），控制 X2 五向钉住第 94 轮那句 `md=94 / html=93` 的绿。`check_table_overflow.py` 把 NOHYDRATE/NOURL/FETCH-FAILURE 与逐页 served≠authored 一起挪进覆盖桶（`content-problems` 与 `coverage-gaps` 分开印，任一非零都不给 0 退出码，`--selftest` 钉住这本账），这条逐页对平当场量出更新日志页 979 对 981，归因到裸 `split("|")` 把行内代码里的转义竖线当成格边——读者没丢任何格子，是尺子多算了 2 格，两处解析一并改掉后八页逐页等齐；本轮另留一笔：`run_battery.py` 不认 `--help`，会把整套电池跑一遍。
...
### 七、复验在上一节写完之后落地，而两条腿给出的答案相反：取不到轮次的那条不能说读者没拿到

先给常驻判据的读数（`--live --page`，与本轮开头那条 `prediction-mismatch=1 served_markers=2` 是同一处、同一把尺的反向）：

```text
live leg: pages=1 (single page: 00-index/changelog.md, of 196 indexed) fetch-failures=0 prediction-mismatch=0 served_markers=0 bold_pages_lost=0 bold_spans_lost=0 (of 2686 authored pairs) words_not_in_served_page=0 bold_spans_atom_only=5
```

`exit=0`。侧栏另走一条按**文字**匹配的浏览器腿（不按 href：第 93 轮那次 `NO-SUCH-ANCHOR` 就是水合后 href 被改写造成的），三个方向一起要求——新条目恰好一条、其中没有星号、旧条目一条不剩：

```text
nav anchors in DOM=742
new entry hits=1 ['三、第三条腿：作者写的加粗标记到读者页面上到底有没有变粗']
old entry hits=0 []
STAR-ON-SCREEN no  /  OLD ENTRY GONE yes
```

`hits=0` 那一行是这条证据的全部价值：只证「新句子在场」是不够的，侧栏完全可以长出第二行而读者照旧看见星号。DOM 里的锚点数从改前的 727 涨到 742，也顺手说明探针量的仍是活的目录而不是空页。

**而同一时刻 `check_live_sync` 的 HTML 轮次腿仍然短读**（`no closing </html>`，本轮第四次在活体上开火），于是它「说不出这一页翻到第几次」。这条不对称必须写下来，因为它很容易被读反：**一条腿取不到轮次，不等于读者没拿到修复**。本轮若拿 `--watch` 的绿、或拿轮次腿的沉默当收尾判据，前者会假绿（页面确实晚到），后者会把已经上线的修复记成未上线。读者可见面的判据只能是读者侧那两条——常驻的等式腿，加按文字匹配的目录腿；轮次腿只负责「平台发没发」，它沉默时不许外推到内容。

顺带一条归因：`words_not_in_served_page` 在这一页单独量是 `0`，而全站那趟是 `2`——§五 ③ 那两格不在更新日志里，留给下一轮按页定位。
...
第 31 轮立过一条整合轴——「新页的核心概念要登记进 [术语表](../21-glossary/README.md)」，当时给睡眠时计算/长上下文/推理预算三页补了词条。但第 42 轮（Text-to-SQL）与第 44 轮（Embedding 微调）是**那之后**加的新页，我没回头补登记——本轮自查发现这俩页的关键词在术语表里**全 0 命中**（Text-to-SQL、Schema 链接、难负例、InfoNCE/对比学习、Embedding 微调），是**我自己留下的整合缺口**。
...
承第 30 轮的整合轴，换一个没量的子维度：**新页的核心术语有没有进 [术语表](../21-glossary/README.md)**。术语表是全书的规范词条索引，一个新概念若只在正文出现、词条表里查不到，读者速查时就漏了它。量：`Sleep-time / 有效上下文 / context rot / test-time compute / 推理预算` 在术语表里**全部 0 命中**——三张新页一个词条都没登记。
