...
四处 `||` 粘连的行与两段「只有行带、没有表头」的行带，一共把 18 行术语降级成段落或直接吞掉——缺表头的两段各自并回上一张真表，粘连行拆开，两处重复术语并进规范行（两条解释都保留，一个字没删）；修法按同一页的渲染差分确认（表格行 111 → 127，六条抽到的术语从「只在原文出现一次」变成各自占一格），再拿两张真实渲染图逐条看过。这类缺陷离线「文字在不在」永远看不见，所以判据常驻进 `check_structure.py` 的新 `TBLORPHAN` 腿：展示公式里的竖线由两条轴共用的同一个分词器豁免（首趟就在 08 章误伤 2 行，改的是尺子不是书），绿色由种植反例加一条「把真页的表头删掉就必须响」的文件派生控制换来。
