...
- **红→绿是量出来的，不是推的**：同一把尺子跑 `HEAD` 版本的这两个文件 → `findings=1 [('騙', 19)]` 与 `findings=1 [('來', 11)]`；跑修完的工作区 → **0**。最终全站读数 `pages=198 prose CJK chars=317568 traditional-form findings=0`。
...
- **仍未覆盖的部分**：抽样只有 7 页，而全站含组件页 **175** 页（这条轴的清单跑出来的读数）；全量线上对平要等浏览器视口问题解决之后再谈，否则一次要人工点 175 次导航。
