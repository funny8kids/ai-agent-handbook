...
- **补齐违反「每页有图」自订标准的页面**：[Lab 2](../19-labs/lab2-rag.md) BM25 检索链路、[Lab 3](../19-labs/lab3-mcp.md) MCP 握手时序、[Lab 5](../19-labs/lab5-eval-trace.md) 评测—trace 数据流、[Lab 6](../19-labs/lab6-guardrails.md) 两道护栏决策流，各配一张章色 Mermaid；新图全部经 Edge 真实渲染逐张目检（其中 4 张首版布局有裁切/留白问题，改扁平 LR 后重渲通过）。
