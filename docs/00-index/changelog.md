...
## 2026-09-23（第 57 次）把上一轮的「结构对平」轴**从 7 页扩到全站 174 页且不需要浏览器**：读数 problems=10 → **0**，其中 2 条是读者真能看到的公式缺陷、5 条是尺子自己的口径错、3 条是网络抖动；并第一次把 README 挂在嘴边却从未入库的**公式判据脚本落进仓库**

第 56 轮那条轴只跑到 7 页，因为它假设「线上 DOM 只有浏览器能碰」。本轮没有先去做浏览器的事，而是先去量**服务端吐的 HTML 里到底有没有这两个结构**——结果两条等式（标签页、公式）在 SSR 阶段就已经等于作者写法，于是全站审计变成一次 urllib 抓取。扩覆盖的收益不是数字好看，是它**当场抓出 10 条**，其中两条是读者屏幕上真实多出来的两个美元符号。
...
| 堆 | 条数 | 内容 | 处置 |
|---|---|---|---|
| 真缺陷（读者可见） | 2 | 展示块里嵌套 `$$x$$`：`communication-protocol` 与 `prompt-injection` 各一处 | **改内容** + 给判据加 NESTED 类 |
...
### 二、抓出来的 2 条真缺陷：展示公式里嵌套 `$$`，读者看到字面美元符号

- **它长什么样**：`$$` 展示块内部又写了一对 `$$n$$`（想强调「n」）。**平台按顺序配对定界符**，所以那一对不会开启新公式，而是让整块在 `\text{` 处提前收尾——线上渲染出的文字里**原样留着 `$$n$$`**。这是本轮抓到的第一类「**内容页首屏之外的公式**」缺陷：等式判据报出的 `authored=14 served=13` 就是它，纯文本存在性探针看不见（字确实在页面上）。
...
- **红→绿是量出来的**：同一把尺子跑 `HEAD` 版本的这两个文件 → `findings=2`（两条 NESTED，逐条点名到行）；跑修完的工作区 → **0**。并且这两页现在**逐条源码级对平**：`authored=13 served-annotations=13`、`authored=19 served=19`，无一缺失、无一多余。
...
- **一处自查**：第三节那个 `re.M` 缺失，是我在**已经宣布「全站跑通了」之后**才由控制项撞出来的。上一轮刚记过「把『我以为我量了』当成『我量了』」，这一轮的形式换成「我以为我改对了」——同一条教训的第二种犯法。
