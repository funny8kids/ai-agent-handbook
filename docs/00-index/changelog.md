...
1. **线上默认正文列宽是 768px，不是 1120px。** 页面 markup 是 `<main class="… max-w-3xl … layout-wide:max-w-6xl …">`，`layout-wide` 默认不在祖先上；段落实测 707px。1120 恰好是 `max-w-6xl`（1152）减内边距，也就是**宽版布局**的列宽——第 24 轮以来一直拿它当默认，README 据此写着「最宽 1116px，全部落在正文列宽内，线上不会被缩放」。`.gitbook.yaml` 里没有宽度项（只有 `root` 与 `structure`），所以这条改不动仓库。
