...
- **第 65 轮按这条轴逐张重排**：操作者拍板「分批逐张重排」，批 1 是读数最险的 5 张（首页封面 + `03-context-budget` / `04-react-loop` / `05-mcp-architecture` / `07-plan-vs-react`）。等比缩放中性，所以只有两个自由度：**窄画布**（封面 1920→1280，缩率 0.4→0.6）与**抬字号 + 长画布**（正文图最小字号抬到 15px，0.8 缩率下正好 12.00px；装不下就往高度长，缩放只看宽度所以高度免费）。全站读数 **32 张 / 956 个 → 27 张 / 734 个**，五张图 223 个越线标签清零，`--column 1152` 的 what-if 23 张 / 378 个 → 18 张 / 204 个；**内容一字未删**（只有 3 处措辞被压短，逐条记在更新日志第 65 次）。轴照旧 `exit 1`，因为还剩 27 张
- **读者拿到的不是仓库里那份 SVG**：GitBook 过资产时跑 sanitizer——重排属性、删注释，并且**剥掉 `paint-order`**。于是 `fill + stroke + paint-order="stroke"` 这种「描边当光晕」的标签在线上变成 3.5px 淡色描边压在 15px 字形上，读者看到一片浅灰残影，而离线字号轴读数完全正常（字号确实是 15px）。第 65 轮线上抽查真渲染才抓到，共 5 处（`04` 3 个、`07` 1 个、`19-lab` CSS 1 个），改成**两层 `<text>`**（底层 `fill="none"` 只画描边，上层只写 `fill`），全是普通表现属性。判据落库 `tools/checks/check_svg_sanitizer.py`：离线腿（不联网，任何 authored 资产依赖 `KNOWN_STRIPPED` 里的特性即红）+ 线上腿（逐张取页面上的 `~gitbook/image` 代理地址——`.gitbook/assets/*` 直接 404，拉回服务端副本比对元素与属性多重集，并断言 `viewBox` 相同、副本仍可解析）。控制：selftest 种的属性版与 CSS 版 `paint-order` 都必须被抓、两层 text 的干净写法必须 0 命中；比对数 <25 直接失败，「干净」不许来自空样本。本轮读数 依赖被剥特性 **1 → 0**
