...
*《图：五个分支彼此一条连线都没有，说明它们是并列部件而非先后阶段；自主 Agent 与提示链同列同级，按下一步可否预知来挑》*
...
$$
\text{下一步可预知？}\;\times\;\text{需要运行时信息？}
$$

| 可预知 | 需要运行时信息 | 选择 | 理由 |
|---|---|---|---|
| 是 | 否 | 固定工作流 / 顺序链 | 最便宜、可测试、可估成本 |
| 是 | 是 | 路由 + 固定分支 | 用规则或分类器选分支 |
| 否 | 是 | Agent 循环（ReAct） | 只能边做边决定 |

关键判据是**成本与可预测性的交换比**：

$$
\text{Agent 的收益}=\text{处理未预设情况}\qquad
\text{Agent 的代价}=\text{路径不可预测}+\text{成本不可估}
$$

因此工程上的默认起点是「能用固定流程就用固定流程」，只在确实无法预知下一步时引入自主循环。真实系统常常是混合：外层固定流程（阶段清晰），内层局部 Agent（探索性强）。

判断是否值得引入自主循环，还可以用一个更直接的代价比较：

$$
\text{可预测成本}\times\text{低}\;\;>\;\;\text{不可预测成本}\times\text{高}\;\;\Longrightarrow\;\;\text{选固定流程}
$$

也就是说，当流程本身足够稳定、异常率低时，「不可预测」带来的隐形成本（调试、审计、成本失控）通常超过它带来的灵活性收益。
