...
上界是 $$\min(30.0,\;0.5\times2^{0})=0.5$$ 秒，实际等待在 `uniform(0, 0.5)` 上取值，可能是 0.03 秒，也可能是 0.49 秒。别嫌这点等待短：抖动的作用不在单次恢复快慢，而在第 100 个并发客户端同时失败的那一刻——不加抖动时它们的最坏节奏是 0.5、1、2、4 秒整齐叠加，正好把下游按同一节拍再次打满；均匀取值把这个尖峰摊平成一段噪声（成因与实现见[错误处理、重试与降级](../11-engineering/error-handling-retry-fallback.md)）。
{% endstep %}
{% step %}
...
{% tab title="耗尽：降级或上报" %}
`MAX_ATTEMPTS` 用尽后抛 `FatalError`，文案里直接写着下一条动作。此时应当**先看这段重试的错误类型分布再决定重跑**：如果绝大多数尝试都是同一类 `ValidationError`，那说明问题在参数生成而不是网络，退避再多也只是延后失败。
{% endtab %}
{% endtabs %}
