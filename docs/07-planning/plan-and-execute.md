...
*《图：同一份调研任务、两条泳道——左道第 4 步失败后整条计划回炉重排，右道每步现想；省下的 token 与错过的变化是同一枚硬币》*
## 先看结论
...
*《图：主环节点不是 Planner 而是「结果符合预期?」——是则自圈进下一步，偏差才把控制权交给 Replan 送回执行器；人审那一问本可跳过》*
...
- **不写预期就没有重规划**：演示里跑得很顺的 P&E，到生产上翻车多半是步骤裸奔发出的。经验值上，派发时每步多花十几个 token 写一句「这一步什么现象算通过」，任务成功率就有可见的提升，这也是唯一能让偏差公式落地的办法。
- **警惕重规划风暴**：上线后第一周常遇到另一种极端——接口抖动、鉴权过期这类环境噪声让偏差阈值频频触发，模型两三步就重写一次计划，我们见过一个任务里 planner 被拉去重写了近十次。要加「单任务重规划不超过 N 次、超限升级给人」的硬上限，同时把预期写成可判别的现象而不是状态描述。
- **计划本身也有成本**：planner 要看全局才能拆，复杂任务生成一版计划常是几万输入 token 起步、耗时十几秒，用户盯着空白界面会以为卡死。产品侧先把计划渲染出来再执行，顺带把 HITL 审批的位置也空出来了。
