...
$$
\frac{\partial f_\theta}{\partial(D\text{ 中的指令})}=0
$$
...
一个容易误判的新情况：2026-09 出现的 [System One 决策模型](../18-frontier-2026/system-one-decision-models.md)（只做判断、不生成文本，常被拿来当执行前风险门）依然属于 $$\mathcal{S}$$，**不能因为它「不是对话模型」就当成硬层**。但它改变了注入的收益结构：这类模型没有行动能力，注入无法借它执行任何东西，攻击目标从「夺取行动」退化为「**操纵判定**」——把风险门判成「安全」。所以用了它，就必须把「判定」与「执行」分开放：判定层可以被污染，执行层的权限边界与执行后核验不能省。
