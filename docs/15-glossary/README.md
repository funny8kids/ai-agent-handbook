...
- [ ] 拿到任一术语能立刻说出它属于哪个主题组（模型训练 / 工具协议 / 记忆检索 / 多智能体工程 / 基础设施 / 具身智能）
- [ ] 中英文双向映射不打岔：读到 PTC、WBC、Goodput 能对上中文，反之亦然
- [ ] 分清易混对：prefill 与 decode、TTFT 与 TPOT、连续批处理与静态批处理、Agent Harness 与「框架」
- [ ] 进入陌生章节前先来这里扫掉核心术语，正文里不再被名词卡住

> 本章是纯查表页，不出自测题——取而代之，记住用它的三条路：

## 用术语表的三条路

- [ ] **读前预习**：开读 16/17 这类术语密集章前，先查对应主题组（如「基础设施与部署」「具身智能」）过一遍
- [ ] **读外网反查**：看英文厂商文档/论文时，用英文列反查本书统一译名，避免同一概念三种叫法
- [ ] **写作对齐**：给本书或团队文档贡献内容时，术语首次出现按本页格式给「中文（English, 缩写）」对照，不自造译名
