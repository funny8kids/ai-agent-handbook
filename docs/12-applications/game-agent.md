...
## 核心机制：游戏是 Agent 的低成本试验场

游戏环境同时满足三个条件，使它成为 RL 与 Agent 研究的理想场地：

| 条件 | 为什么关键 |
|---|---|
| 可重置、可重放 | 环境能回到初始态，试错成本接近零 |
| 奖励程序化可验证 | 胜负、得分、通关都是客观信号，不需要人工打分 |
| 状态完全可观测 | 不需要从自然语言里猜环境状态 |

单位训练成本可以粗略写成：

$$
\text{成本/episode}=\frac{\text{rollout 成本}+\text{环境运行成本}}{\text{episode 数}}
$$

游戏里这两项都远低于真实世界（无物理损耗、可并行多实例），因此适合**先在这里验证策略与训练配方，再迁移**。对应到数字 Agent 的启示：**凡是有客观可验证信号的任务，都适合做强化学习**（见 [RLHF、DPO 与对齐](../03-llm/rlhf-dpo-alignment.md)）。

需要提醒的是迁移并不免费：游戏环境的状态表示与真实任务差异很大，直接在游戏上练出的策略往往不能原样搬到业务场景。可迁移的是**训练配方与评测方法**（奖励设计、课程难度、评估指标），而不是策略本身——这与具身智能里的 sim-to-real 是同一类问题（见 [仿真与 Sim-to-Real](../17-embodied-ai/simulation-sim2real.md)）。
