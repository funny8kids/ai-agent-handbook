...
- **pi-mono 的 Slack bot harness**（[badlogic/pi-mono](https://github.com/badlogic/pi-mono)）：把 Agent 接入 IM 作为「随身助理」的轻量参考——消息即界面，工具即能力
- **Claude Code 的 CLAUDE.md 启示**：项目记忆文件的「用户可编辑 + 自动加载」模式，同样适用于个人偏好记忆——用户能直接看、改自己的「AI 认知」，是记忆透明化的好范式
- **OpenClaw 类个人 Agent 生态**（基于 pi-mono 构建，见 [pi-mono 文档](https://github.com/badlogic/pi-mono)）：2026 年个人 Agent 热潮的代表——「常驻 + 多渠道 + 主动服务」的完整形态
