...
*《图：Evaluator 判失败才惊动 Reflector——它把「为何失败/怎么改」写进记忆，带教训重新回到 Actor 再跑；判成功则直接输出，权重自始至终没动》*
...
- **想清楚复盘存在哪一层**：只放当前上下文 = 任务内 Reflexion，换个任务就忘；按「任务类型 + 失败签名」落库、下次同类任务再检索回来，才升级成跨任务的长期能力——代价是必须处理过期教训，环境一变旧复盘就成了误导。
- **入库前先按失败签名去重**：同一类错误（缺依赖、超时、断言写错）合并成一条，比堆五条相似复盘更省上下文，也更不容易把偶然归因写成规律。
