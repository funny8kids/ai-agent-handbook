...
flowchart TD
    ...
    T["任务：做什么"] --> X
    C["约束：不许做什么"] --> X
    E["示例<br/>输入到输出的示范<br/>最强条件信号"] --> X
    F["格式：输出长什么样"] --> X
    X --> M["LLM 按条件分布<br/>逐 token 采样"]
    M --> O["输出<br/>x 越准越贴近目标"]
```
...
- **规则是往上堆还是往下删**：我们常见的项目里，约束到八条上下还守得住；加到二十条以后，新塞进去的几条几乎不生效，模型稳定违反的往往是最后加的那两三条。所以 prompt 评审里最有用的一句话是「这条能删吗」。
- **别拿十条样本下结论**：措辞、示例顺序、标点都能让结果摆动十几个点。经验值上，同一模型同一版本至少几十条固定输入，才有资格说「我优化好了」——上线前留一份基线输出，比对才有个锚；改完最好再跑两轮，看波动的方向一致再谈上线。
- **谁半夜改的 prompt**：{% hint style="warning" %}现场最常见的事故不是模型变笨，是有人在线上补了一句「请务必……」，三天后没人记得它当初挡的是什么。本文「Prompt 是代码」那句话，落到操作上很朴素：进 PR、跟回归集一起提交，改动理由写在 commit 里。{% endhint %}
