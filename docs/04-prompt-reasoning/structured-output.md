...
词表按 $$10^5$$ 量级算，但解码第 1 步合法的 token 只有 `{`（以及带换行/空格的变体），候选集从 $$10^5$$ 当场缩到个位数。轮到 `category` 的值：模型想写「扣费」也合法吗？不合法——此刻前缀已进到 `"b`，枚举自动机只认 `illing"` 这一条延续，中文 token 的概率被置零后重新归一化（就是第 1 节那条公式）。**模型的概率质量被摊到语法可行的候选上，所以合法率是 100%，但选中的那个值未必对**。
{% endstep %}
{% step %}
