...
*《图：反馈—修改自成一个环——每轮改完再回炉自评，出口是「质量达标或撞到迭代上限」二选一；因为全程不引外部信息，到不动点后继续转只烧 token》*
...
| 档位 | 反馈来源 | 每轮成本 | 什么时候用 |
|---|---|---|---|
| 纯自评 | 同一模型按维度打分 | 1 次生成 + 1 次批评 | 格式、完整性、风格类任务，且没有客观验证器 |
| 自评 + 外部验证 | 批评之前先跑 lint / 测试 / schema 校验 | 1 次生成 + 验证 + 1 次批评 | 有代码、有结构化输出的任务——默认选这档 |
| 自评 + 检索 | 用检索到的原文核对事实 | 再加 1–2 次检索 | 事实性写作，模型记忆不可靠时 |
| 交叉互评 | 换一个模型扮演 $$F$$ | 两个模型的调用 | 单一模型自评明显偏乐观时（长文摘要、多需求清单） |

**选择规则**：反馈越外部、越客观，精修的质量上限越高。纯自评只能兑现「模型本来会、这一轮没做好」的部分；一旦任务存在可执行的判据（测试能跑、格式能校验、原文能检索），就该把 $$F$$ 交给判据而不是交给模型的自我感觉——判据不会迎合模型，自我批评会。
