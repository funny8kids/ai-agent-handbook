...
第 1 层的 0.82 与 0.61 都还在（0.61 恰好过线），账面上看不出变化。但每往下一层，低于 0.6 的候选一律出局，`beam=2` 就常常只剩 1 条路——**它的脆性与 `beam=1` 同源，而且「整层为空」的概率随深度累积**，第 4 步那个空集陷阱更容易被踩到。抬高及格线只在评估器确实准（有外部验证器给分）时才划算，否则换来的是同一条路径上的假阴性率上升。
{% endtab %}
{% endtabs %}
