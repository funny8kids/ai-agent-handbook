...
- **Pi：ReAct 的最小工程体**（[earendil-works/pi](https://github.com/earendil-works/pi)）：agentLoop 把 ReAct 循环压缩到 ~300 行——模型消息里的 tool_use 就是 Action，工具执行结果回填就是 Observation，而「Thought」直接由模型的原生推理承担。读完这个仓库，再厚的框架都不神秘
