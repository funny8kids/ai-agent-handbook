...
6. **细条不等于看不清**：W < 420 的竖排链文字仍是 16px，代价只是正文右侧留白。全库现存 9 张这类竖排图（时间线、单链流程），属可接受的版式取舍，不算可读性缺陷。
...
- [ ] frontmatter 四字段齐全且顺序固定：`tags` → `type` → `status` → `updated`（`SUMMARY.md` 与本模板目录下的示例除外）
