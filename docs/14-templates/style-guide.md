...
Mermaid 块不强制配图注，但**必须被实质文字解释**，二选一即可：图前一句说明「这张图在讲什么机制」，或图后一句说明「读完要带走什么」。全书现状：132 块带 `*《图：……》*` 图注，48 块由实质引导语/后置正文解释。
...
全书 Mermaid 块统一带**章色主题指令**：每个 ```` ```mermaid ```` 块的第一行必须是 `%%{init: ...}%%`，取所在章节的主色，让读者扫一眼就知道自己在第几章。2026-09-20 起全书块已全量注入（现 180 块，新增块同步注入），新增图不得省略。
