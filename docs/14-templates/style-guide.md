...
全书 Mermaid 块统一带**章色主题指令**：每个 ```` ```mermaid ```` 块的第一行必须是 `%%{init: ...}%%`，取所在章节的主色，让读者扫一眼就知道自己在第几章。2026-09-20 起全书 154 块已注入（19 章新增块同步注入），新增图不得省略。
...
GitBook 会把 Mermaid 的 SVG 缩到正文列宽（约 1120px）：图越宽，字越小。但**图太窄同样不合格**——竖排长链会塌成 168–410px 的细条，正文右侧空一大片。两个方向都要量，量的是每块的自然宽 W 与自然高 H：`W > 1120` 判超宽，`W < 420 且 H > 380`（宽高比 <0.55）判细条。2026-09-22 全库 170 块实测出三条规律：

1. **`flowchart LR` 的宽度由「排数」决定，不由标签长短决定**。6 排链无论怎么精简措辞都稳定在 1128–1200px，5 排约 936–1080px；要变窄只能无损合并节点，改文字无效。
2. **`flowchart TB`/`TD` 的长链必然塌成细条**，而 `subgraph … direction LR` 在本 Mermaid 版本被忽略（实测仍渲染成 304×829 的竖排）。唯一有效的解法：父图写 `flowchart LR`，把流程拆成两个**互不相连**的 subgraph——只要留一条跨排边，dagre 就把两排横向拼接（5+5 排一拼就 1600px 以上）。被删掉的那条跨排边必须写进图注，信息不得丢。
3. **父图为 LR 时，后声明的 subgraph 渲染在上面**，所以源码里逻辑上的第二排要写在前面；改完必须截图复核读序。

另外：rank 数不等于最长路径——旁支与孤立节点各占一排，别按「最长路径 ≤5」批量翻转方向（本轮这样批量改 16 张，15 张超宽后回退）。判断一张图合不合格，要截**线上页面**的图看，本地能渲染不等于线上能看清。
