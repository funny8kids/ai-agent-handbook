...
GitBook 会把 Mermaid 的 SVG 缩到正文列宽（约 1120px）：图越宽，字越小。因此横向 `flowchart LR` 最多 6 个节点，更长的流程一律改竖排 `flowchart TD`——竖排自然宽度通常只有 500px 左右，不会被缩放，每张卡片的文字都清晰。别指望用 `subgraph` 里再写 `direction TB` 做泳道来救宽度：只要有一条边跨过 subgraph 边界，Mermaid 就忽略内部 `direction`，版式照样塌成窄条。判断一张图合不合格，要截**线上页面**的图看，本地能渲染不等于线上能看清。
