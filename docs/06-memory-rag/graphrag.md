...
**社区划分**用模块度最大化（Leiden 算法）。模块度 $$Q$$ 衡量「社区内连接是否比随机情况更密」，定义式与符号（邻接矩阵、节点度、总边数）见 [知识图谱](knowledge-graph.md)。社区内部越紧密、社区之间越稀疏，$$Q$$ 越大。划分出的社区再按层次组织（大社区含小社区），摘要也分层生成。
...
*《图：两条链的结构差别只在那个菱形判断上——问关系取实体邻域子图，问全貌才付出社区摘要 map-reduce 的代价；左边那条根本没得选》*
