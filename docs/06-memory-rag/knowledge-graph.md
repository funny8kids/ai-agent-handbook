...
$$k$$ 跳就是把连接重复 $$k$$ 次。这在图数据库里是一条 Cypher/SPARQL 查询，返回的**路径本身**就是可展示的解释。查询形态（不是要跑的脚本，注意它约束的是"相邻两跳必须共享一个实体"）：
...
而返回值才是关键——它天然带着"为什么给出这个答案"的证据链，可以直接喂给模型或画给人看：

```json
{
  "answer": "华辰电子",
  "path": ["张三", "ACME", "芯片A", "华辰电子"],
  "hops": 3,
  "edges": ["就职于", "生产", "供应"]
}
```

向量检索只能返回"语义相近的块"，说不出中间经过了谁；图谱这一跳一跳的 `path` 是可核查的——任一条边不成立，答案就能被指着撤回。这也是 GraphRAG 愿意多维护一套图结构的根本理由。
...
*《图：三条实线边各自都是一条可核对的三元组，虚线那个二跳推理并不存在图里，它是沿前三条边连走出来的路径》*
