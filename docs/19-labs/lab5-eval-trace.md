...
```json
{"case": 1, "answer": "4800 万元", "pass": true, "spans": [{"step": "plan", "start": 0.0, "dur": 0.022788535969157673}, {"step": "tool:search", "start": 0.022788535969157673, "dur": 0.052501075522266696}, {"step": "answer", "start": 0.07528961149142437, "dur": 0.02825087955107358}]}
```

**要点**：`start` 是相对用例起点的偏移，`dur` 是这段耗时——只要有这两个数，任何工具都能重建瀑布图，不需要 SDK。这就是第 11 章讲的 span 数据契约的最小形状。

一句对照说明：这行是本机一次真跑的原文，而且**今天重跑还是这一行**——`dur` 由脚本里的随机种子钉住（22.8 / 52.5 / 28.3 毫秒），`start` 走的是**模拟时钟**（上一条的 `start + dur`），所以第一段从 `0.0` 起算、每段恰好接在上一段之后。这里要交代一句改过的历史：第 54 轮的日志量到 `start` 当时读的是真实时钟，`sleep` 回来的那点余量每次不同，于是这一行**和下面瀑布图的一格缩进会逐次漂移**，页面那时只能提醒读者「别去对那串 16 位小数」。第 80 轮把它改成模拟时钟并落了判据（`tools/checks/check_lab_runnability.py`：六个脚本各连跑两遍，要求逐字节相同，且 `labs/out/*.out` 必须等于今天重跑的输出），现在这串小数是可以对的。
{% endstep %}
